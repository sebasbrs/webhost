...
class Forum extends MX_Controller {
    ...
    {
        $data['title'] = $this->lang->line('nav_forums');
    ...
    public function topic($id)
    {
        ...
        if ($this->forum_model->getType($this->forum_model->getTopicForum($id)) == 2 && $this->m_data->isLogged())
            if ($this->m_data->getRank($this->session->userdata('ci_sessions')) > 0) { }
        else
            redirect(base_url('forum'),'refresh');
    ...
    public function newTopic($idlink)
    {
        ...
        $this->forum_model->insertTopic($idlink, $title, $this->session->userdata('ci_sessions'), $description, $lock, $highl);
    }
}
